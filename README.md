...
python v1.3 이상의 구현체에서는, 
기본 라이브러리를 지원한다 맨이야.  
다만, 라이브러리를 만드는 것은 지원되지 않는다 맨이야.

## 포함하는 법

라이브러리를 포함하는 방법은,
'동인천의' 뒤에 해당하는 이름을 적으면 된다 맨이야.  
해당 문장 이후부터 해당 라이브러리의 기능을 사용할 수 있다 맨이야.
...
기본적으로 모든 라이브러리의 기능(함수)는,
(라이브러리 이름)#(함수 이름)의 형태를 띈다 맨이야.  

```
코 "미션" 라이브러리의 "추가" 기능 사용
동인천의 미션
미션#추가(0)
```
...
"미션" 라이브러리는,
케인인님의 방송(코드)에 미션(리스트)을 추가하는 라이브러리다 맨이야.  
미션은, 파이썬의 list와 거의 동일하다 맨이야.  
다만, 따로 미션 변수를 선언할 수는 없고,
코드당 하나 씩의 미션만 있을 수 있다 맨이야.

또한, 미션은 정수만 담을 수 있고, 전역 변수이기 때문에,
함수 안과 밖에서의 상황을 공유할 수 있다 맨이야.

```
코 미션 라이브러리의 기능들
동인천의 미션

코 미션의 맨 뒤에 새로운 요소를 추가
미션#등록(0)

코 미션의 맨 뒤에 여러개의 요소를 추가
미션#등록(1, 2, 3, 4, 5)

코 미션의 맨 뒤의 요소를 삭제하면서 반환
미션#성공()

코 미션의 1번 인덱스의 요소를 삭제하면서 반환
미션#성공(1)

코 미션의 2번 인덱스의 요소를 확인
미션#수락(2)

코 미션을 '1 2 3' 과 같은 형식으로 출력
미션#
```
...
코랭의 형식을 지키지 않은 경우
* `뭐 이런 그지 문자가 다 있어?`  
인식할 수 없는 문자를 사용했을 경우
* `있어야 할게 없잖아 임마!`  
동인천 룰상 꼭 있어야 하는 무언가가 없을 경우
* `너 지금 뭐라고 했니?`  
방송이 동인천 룰을 만족하지 않을 경우
* `인 방송 중에는!`  
방송 진행 도중에 이거나드셔가 난 경우
